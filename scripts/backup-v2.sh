#!/bin/bash

# 项目备份脚本 v2
# 用途：创建完整的项目备份，使用 rsync 排除不必要文件
# 使用方法：./scripts/backup-v2.sh [备份目标路径]

set -e  # 遇到错误立即退出

# 获取脚本所在目录
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"

# 备份目标路径（如果未指定，使用当前目录）
BACKUP_DIR="${1:-$PROJECT_ROOT}"
TIMESTAMP=$(date +"%Y%m%d_%H%M%S")
BACKUP_NAME="my-tools-backup_${TIMESTAMP}"
BACKUP_PATH="${BACKUP_DIR}/${BACKUP_NAME}"

echo "=========================================="
echo "项目备份工具 v2"
echo "=========================================="
echo "项目路径: $PROJECT_ROOT"
echo "备份目标: $BACKUP_PATH"
echo "时间戳: $TIMESTAMP"
echo ""

# 创建临时备份目录
TEMP_BACKUP="/tmp/${BACKUP_NAME}"
rm -rf "$TEMP_BACKUP"
mkdir -p "$TEMP_BACKUP"

echo "正在收集文件..."

# 使用 rsync 复制项目文件，排除不必要的目录
rsync -av --exclude='node_modules' \
          --exclude='.wrangler' \
          --exclude='.astro' \
          --exclude='dist' \
          --exclude='.git' \
          --exclude='coverage' \
          --exclude='*.log' \
          --exclude='.DS_Store' \
          --exclude='.env' \
          --exclude='.env.local' \
          --exclude='.dev.vars' \
          "$PROJECT_ROOT/" "$TEMP_BACKUP/" > /dev/null

echo "文件收集完成"

# 创建备份信息文件
echo "  - 备份信息"
cat > "$TEMP_BACKUP/BACKUP_INFO.txt" <<EOF
项目备份信息
========================================
备份时间: $(date)
备份版本: $(cd "$PROJECT_ROOT" && git rev-parse HEAD 2>/dev/null || echo "未知")
Node 版本: $(node --version)
npm 版本: $(npm --version)

备份内容:
- 源代码 (src/)
- 配置文件 (package.json, tsconfig.json, etc.)
- 脚本文件 (scripts/)
- 文档文件 (*.md)
- Workers 目录 (workers/, cron-worker/)
- Git 配置 (.gitignore)

排除内容:
- node_modules/ (可通过 npm install 恢复)
- dist/ (构建产物，可通过 npm run build 生成)
- .wrangler/ (Wrangler 缓存)
- .astro/ (Astro 缓存)
- .git/ (Git 历史，如需完整历史请单独备份)
- .env, .env.local, .dev.vars (敏感信息)

恢复步骤:
1. 解压备份文件
2. 进入项目目录
3. 运行: npm install
4. 运行: npm run build (如需要)
5. 配置环境变量（参考 DEPLOYMENT.md）

重要提示:
- 备份不包含敏感信息（如 .env 文件）
- 备份不包含 Cloudflare KV 数据（需单独备份）
- 备份不包含用户数据（存储在 Cloudflare KV 中）
EOF

# 创建压缩包
echo ""
echo "正在创建压缩包..."
cd /tmp

tar -czf "${BACKUP_NAME}.tar.gz" "$BACKUP_NAME"

# 复制压缩包到目标位置
cp "${BACKUP_NAME}.tar.gz" "${BACKUP_PATH}.tar.gz"

# 清理临时文件
rm -rf "$TEMP_BACKUP"
rm -f "${BACKUP_NAME}.tar.gz"

# 显示备份信息
FILE_SIZE=$(du -h "${BACKUP_PATH}.tar.gz" | cut -f1)
FILE_COUNT=$(tar -tzf "${BACKUP_PATH}.tar.gz" | wc -l | tr -d ' ')
echo ""
echo "=========================================="
echo "备份完成！"
echo "=========================================="
echo "备份文件: ${BACKUP_PATH}.tar.gz"
echo "文件大小: $FILE_SIZE"
echo "文件数量: $FILE_COUNT"
echo ""
echo "备份内容摘要:"
echo "  ✓ 源代码 (src/)"
echo "  ✓ 配置文件"
echo "  ✓ 脚本文件"
echo "  ✓ 文档文件"
echo "  ✓ Workers 目录"
echo ""
echo "恢复步骤请查看备份文件中的 BACKUP_INFO.txt"
echo "=========================================="
